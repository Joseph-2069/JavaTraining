...
public class CallableEx2WhileCondition {
	...
	//  ExecutorService ser1 = Executors.newSingleThreadExecutor();
	 ...
	 //  f.cancel(false); //cancel the task if not started, if task started already then do not cancel by interruption
	 //  f.cancel(true); //cancel the task if not started, if task started already then cancel by interruption
...
class MyThread implements Runnable
{
	...
	// overriding rule says that overridden method cannot throw any new or wider checked exception
